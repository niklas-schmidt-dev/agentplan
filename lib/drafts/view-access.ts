...
export type ViewResolution =
  { state: "not-found" } | { state: "granted"; draft: Draft } | { state: "password"; draft: Draft };
 ...
 *
 * - owner (signed in, live draft) → granted
 * - public                    → granted
 ...
 * - missing / deleted / expired / no version → not-found
 */
export function resolveDraftView(
  ...
  if (
    ...
    draft.deletedAt ||
    (draft.expiresAt && draft.expiresAt.getTime() <= Date.now())
  )
