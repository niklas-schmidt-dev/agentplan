...
Media is disabled by default. `AP_ENABLED_UPLOAD_KINDS=html,image` enables raster
images; add `video` only after the checks below pass. HTML always uses the
existing bounded multipart endpoints.
...
browser-generated Content-Length satisfies the presigned request before
enabling R2 media.
