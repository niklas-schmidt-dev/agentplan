...
AgentPlan hosts **arbitrary, hostile HTML and untrusted media uploaded by users
and their agents**.
...
2. **All content lives in private object storage** (private Vercel Blob or a private
   Cloudflare R2 bucket), regardless of draft visibility. Visibility is an
   application-level authorization decision made server-side on every request.
   Private drafts return `404` to non-owners and are served with
   `Cache-Control: private, no-store`. The Blob adapter explicitly requests
   private access and fails closed when connected to a public store.
3. **Direct media capabilities expose staging keys only.** Signed PUT URLs are
   short-lived and bound to one key. Completion copies to an unexposed final key,
   then validates magic bytes, MIME, exact size, image dimensions, and SHA-256
   before recording a version. Staging cleanup remains durable through capability
   expiry, so a late upload cannot permanently resurrect an orphan.
4. **Password-protected drafts** store the password only as a salted scrypt hash.
   A correct entry mints an HMAC-signed, draft-scoped, HttpOnly access cookie; the
   content is served only to the owner or a request bearing a valid cookie for
   that specific draft, and is never publicly cached. A grant for one draft cannot
   unlock another.
5. **API tokens are never stored or logged in full.** Only a visible prefix and a
...
6. **Protected URLs do not disclose titles.** Private and password-protected drafts
   use high-entropy random slugs. A public-to-protected transition rotates the slug.
7. **Initial administration is operator-authorized.** An empty database accepts
...
8. **Email/password identities prove mailbox control.** New accounts receive no
